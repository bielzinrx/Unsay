...
public final class DeleteAnimation {
    ...
    public static void render(PoseStack pose, float delta) {
        ...
        while (it.hasNext()) {
            ...
            drawTrashBody(pose, a.trashX, a.trashY, lidOpen, openPop, squash);
            ...
            if (t < 1f) {
                ...
                GuiDraw.drawString(pose, font, a.text, -font.width(a.text) / 2, -4, color);
                pose.popPose();
            }
    ...
    private static void drawDust(PoseStack pose, Active a) {
        ...
        for (float baseAngle : angles) {
            ...
            GuiDraw.fill(pose, (int) mx, (int) my, (int) mx + 1, (int) my + 1, color);
        }
    }

    public static void drawTrashIcon(PoseStack pose, int x, int y, boolean hovered, float hoverAnim) {
        float h = Mth.clamp(hoverAnim, 0f, 1f);
        if (hovered && h < 0.5f) h = 1f;

        GuiDraw.fill(pose, x - 1, y + 14, x + 15, y + 16, 0x40000000);

        float scale = 1f + h * 0.12f;
        pose.pushPose();
        pose.translate(x + 7f, y + 7f, 0);
        pose.scale(scale, scale, 1f);
        pose.translate(-(x + 7f), -(y + 7f), 0);
        blitTrash(pose, x, y, false, 0.92f + h * 0.08f);
        pose.popPose();

        if (h > 0.01f) {
            int glowAlpha = (int) (h * 110f);
            GuiDraw.outline(pose, x - 2, y - 2, 18, 18, (glowAlpha << 24) | 0x00FFFFFF);
        }
    }

    public static void blitTrash(PoseStack pose, int x, int y, boolean open, float alpha) {
        ResourceLocation tex = open ? TEX_TRASH_OPEN : TEX_TRASH;
        GuiDraw.blit(pose, tex, x, y, 0, 0, 14, 14, 16, 16, alpha);
    }

    private static void drawTrashBody(PoseStack pose, float x, float y, boolean lidOpen, float openPop, float squash) {
        float cx = x + 7f;
        float cy = y + 12f;
        pose.pushPose();
        pose.translate(cx, cy, 0);
        pose.scale(1f + squash * 0.55f, 1f - squash, 1f);
        pose.translate(-cx, -cy, 0);

        ResourceLocation tex = lidOpen ? TEX_TRASH_OPEN : TEX_TRASH;
        float liftY = lidOpen ? (1f - openPop) * -2f : 0f;
        GuiDraw.blit(pose, tex, (int) x, (int) (y + liftY), 0, 0, 14, 14, 16, 16, 1f);
        pose.popPose();
    }
