...
public final class ChatHudEditor {
    ...
    public static void applyEdit(ClientTrackedMessage tracked, String newText, HudPin pin) {
        ...
        GuiMessage replacement = new GuiMessage(m.addedTime(), updated, m.signature(), m.tag());
    ...
    private static int findBestMatchIndex(List<GuiMessage> all, ClientTrackedMessage tracked,
        ...
        if (forcedSig != null) {
            for (int i = 0; i < all.size(); i++) {
                if (forcedSig.equals(all.get(i).signature())) return i;
            }
        }
    ...
    private static boolean looksOwned(GuiMessage g, UUID self, String selfName) {
        ...
        ClientTrackedMessage t = ClientMessageIndex.findBySignature(g.signature());
    ...
    private static int scoreGuiAgainstTracked(GuiMessage g, ClientTrackedMessage tracked,
        ...
        int score = 0;
        if (tracked.signature != null && tracked.signature.equals(g.signature())) score += 1000;
        ...
        ClientTrackedMessage linked = ClientMessageIndex.findBySignature(g.signature());
        if (linked == null) {
